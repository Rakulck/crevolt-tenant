import type { DocumentAnalysisResult, DocumentFile, DocumentType } from "@/lib/types/documents";
...
export class GenericDocumentAnalyzer extends BaseDocumentAnalyzer {
  ...
  private async analyzeGenericDocument(document: DocumentFile): Promise<{
    ...
    try {
      ...
      const completion = await GenericDocumentAnalyzer.openai.chat.completions.create({
        model: "gpt-4o",
        messages: [
          {
            role: "system",
      ...
      try {
        ...
        extractedData = this.extractDataFromText(response, template.extractionFields);
      }
...
Provide a summary and JSON with extracted data.`,
        extractionFields: ["leaseTermYears", "annualRent", "escalations", "keyTerms"],
      },
...
Provide a summary and JSON with extracted data.`,
        extractionFields: ["unitsTraded", "averageRentIncrease", "retentionRate", "marketTrends"],
      },
      rent_roll: {
        systemPrompt: "This should not be used - rent_roll has its own analyzer",
        userPrompt: "",
        extractionFields: [],
      },
      t12: {
  ...
  private extractDataFromText(text: string, fields: string[]): Record<string, unknown> {
    ...
    try {
      ...
      if (text.toLowerCase().includes("increase")) keyFindings.push("Shows increases");
      if (text.toLowerCase().includes("decrease")) keyFindings.push("Shows decreases");
      if (text.toLowerCase().includes("stable")) keyFindings.push("Stable performance");
