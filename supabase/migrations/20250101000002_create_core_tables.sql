...
CREATE TABLE public.user_profiles (
  ...
  CONSTRAINT user_profiles_postal_code_check CHECK (postal_code IS NULL OR postal_code ~ '^\d{5}(-\d{4})?$') -- US ZIP code format
);
...
CREATE TABLE public.tenants (
  ...
  ssn text, -- Social Security Number for tenant identification
  email text,
  phone text,
  emergency_contact_name text,
  emergency_contact_phone text,
  stated_annual_income numeric(12,2) CHECK (stated_annual_income >= 0),
...
CREATE INDEX idx_tenants_stated_annual_income ON public.tenants(stated_annual_income) WHERE stated_annual_income IS NOT NULL;
