...
export const MetadataProcessorService = {
  async processFileForMetadata(file: File): Promise<MetadataResponse> {
    ...
    try {
      ...
      for (const sheet of fileProcessorResult.sheets) {
        try {
          ...
          const metadata: SheetMetadata = {
            ...
            sampleData: sheet.data.slice(headerDetection.headerRow, headerDetection.headerRow + 5),
          };
      ...
      const processingMetadata: ProcessingMetadata = {
        ...
        errors,
        aiCacheKey: MetadataProcessorService.generateCacheKey(file, sheetMetadata),
      };
  ...
  generateCacheKey(file: File, sheetMetadata: SheetMetadata[]): string {
    ...
    const sheetsHash = sheetMetadata.map((s) => `${s.name}_${s.rowCount}`).join("_");
