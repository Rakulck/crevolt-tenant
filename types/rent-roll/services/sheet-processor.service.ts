...
export const SheetProcessorService = {
  ...
  async analyzeSheet(sheet: {
    ...
    try {
      ...
      console.warn(
        `Failed to classify sheet ${sheet.name}, using fallback:`,
        error,
      )
  ...
  fallbackSheetClassification(sheet: Sheet): SheetInfo {
    try {
      ...
      const isSummary = summaryKeywords.some((keyword) =>
  ...
  extractPropertyName(sheetName: string): string | undefined {
    ...
    return propertyName.length > 0 && propertyName !== cleanName
      ? propertyName
      : undefined
  },
  ...
  async processSheet(
    ...
    const extractionResult = DataExtractorService.extractData(
      sheet.data,
      headerDetection,
    )
