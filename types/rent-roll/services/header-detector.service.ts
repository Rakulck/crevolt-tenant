...
export const HeaderDetectorService = {
  ...
  async detectHeaders(sheetData: unknown[][]): Promise<HeaderDetectionResult> {
    try {
      ...
      const aiResult = await HeaderDetectorService.detectHeadersWithAI(sheetData);

      if (aiResult.confidence > 0.7) {
        return {
          headerRow: aiResult.headerRow - 1,
          dataStartRow: aiResult.dataStartRow - 1,
          headers: HeaderDetectorService.extractHeaders(sheetData, aiResult.headerRow - 1),
      ...
      // Fallback detection
      const fallbackResult = HeaderDetectorService.fallbackHeaderDetection(sheetData);
  ...
  async detectHeadersWithAI(sheetData: unknown[][]): Promise<AIHeaderDetectionResult> {
    ...
    for (let row = 0; row < Math.min(10, sheetData.length); row++) {
      ...
      for (let col = 0; col < sheetData[row].length; col++) {
        ...
        if (cell) {
          ...
          if (HeaderDetectorService.FALLBACK_KEYWORDS.some((keyword) => value.includes(keyword))) {
            matches++;
          }
        }
      }
  ...
  extractHeaders(sheetData: unknown[][], headerRow: number): Record<string, string> {
  ...
  mapColumnsToFields(headers: Record<string, string>): ColumnMapping {
    ...
    const fieldMappings = {
      ...
      floor_plan: ["floor plan", "unit type", "layout", "type", "plan", "beds", "baths", "bedroom"],
  ...
  findDataStartRow(sheetData: unknown[][], headerRow: number): number {
    for (let row = headerRow + 1; row < Math.min(headerRow + 5, sheetData.length); row++) {
