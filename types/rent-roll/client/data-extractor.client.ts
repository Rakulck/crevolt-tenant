...
export const ClientDataExtractor = {
  extractSheetData(
    ...
    for (let i = startRow; i < Math.min(endRow, rawSheet.data.length); i++) {
      ...
      try {
        ...
        errors.push(
          `Error processing row ${i + 1}: ${error instanceof Error ? error.message : "Unknown extraction error"}`,
        )
      }
    }
  ...
  calculateSummary(units: RentRollUnit[]): {
    ...
    const averageRent =
      unitsWithRent.length > 0 ? totalRent / unitsWithRent.length : 0
    ...
    return {
      ...
      average_sqft:
        unitsWithSqft.length > 0
          ? Math.round((totalSqft / unitsWithSqft.length) * 100) / 100
          : 0,
      occupancy_rate:
